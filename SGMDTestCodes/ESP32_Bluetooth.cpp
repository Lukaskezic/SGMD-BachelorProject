...
void setup() {
  ...
  SerialBT.begin("ESP32test6"); //Bluetooth device name
...
void loop() {
  ...
  if (digitalRead(PIN2) == HIGH) //Read GPIO 12
  {
    ...
    for(i; i<SAMPLES; i++)
    {
      ...
      SerialBT.println(Voltage0); // 7 = number of meaningful digits
      if(k==0)
      {
         SerialBT.flush(); //Waits until our string has been recieved in the serial terminal
